...
def delete(point):
    ...
    points += [point]
```
因为删除就是添加的逆过程，在一个位置添加一个点和删除一个点对原来的RegionTree造成的影响是一样的

而这样后我们的所有操作会在增加一个$O(lgN)$用来维护一个set，空间消耗很大
update 会被add 调用大概$logS$次，每次大概会加入进$logN$个node中，因此空间复杂度基本上也是$NlogNlogS$的
，也没办法了。
